...
namespace eudaq {
  ...
  StandardPlane::StandardPlane(unsigned id, const std::string &type,
                               const std::string &sensor)
      : m_type(type), m_sensor(sensor), m_id(id), m_tluevent(0), m_xsize(0),
  ...
  void StandardPlane::SetSizeZS(unsigned w, unsigned h, unsigned npix,
                                unsigned frames, int flags) {
  ...
  void StandardPlane::PushPixelHelper(unsigned x, unsigned y, double p,
  ...
  void StandardPlane::SetPixelHelper(unsigned index, unsigned x, unsigned y,
  ...
  StandardPlane::XVector(unsigned frame) const {
  ...
  const std::vector<StandardPlane::coord_t> &
  StandardPlane::YVector(unsigned frame) const {
  ...
  StandardPlane::PixVector(unsigned frame) const {
  ...
  void StandardPlane::SetXSize(unsigned x) { m_xsize = x; }

  void StandardPlane::SetYSize(unsigned y) { m_ysize = y; }

  void StandardPlane::SetTLUEvent(unsigned ev) { m_tluevent = ev; }

  void StandardPlane::SetPivotPixel(unsigned p) { m_pivotpixel = p; }

  unsigned StandardPlane::ID() const { return m_id; }

  const std::string &StandardPlane::Type() const { return m_type; }

  const std::string &StandardPlane::Sensor() const { return m_sensor; }

  unsigned StandardPlane::XSize() const { return m_xsize; }

  unsigned StandardPlane::YSize() const { return m_ysize; }

  unsigned StandardPlane::NumFrames() const { return m_pix.size(); }

  unsigned StandardPlane::TotalPixels() const { return m_xsize * m_ysize; }
  ...
  unsigned StandardPlane::TLUEvent() const { return m_tluevent; }

  unsigned StandardPlane::PivotPixel() const { return m_pivotpixel; }
  ...
  void StandardPlane::SetupResult() const {
    ...
    if (GetFlags(FLAG_ACCUMULATE)) {
      ...
      m_result_pix = &m_temp_pix;
    } else if (m_pix.size() == 16){ // Hexavoard data
      m_temp_pix.resize(0);
      m_temp_x.resize(0);
      m_temp_y.resize(0);
